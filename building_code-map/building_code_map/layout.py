...
#     point_geojson_path = os.path.join(base_path, 'data', 'cleaned_gracy_3-9.geojson')
#     with open(point_geojson_path) as f:
#         points = json.load(f)
#     # NEW: Clean the name fields in points so they do not include '?' or '[*]'
#     import re
#     for feature in points['features']:
#         if 'properties' in feature and 'name' in feature['properties']:
#             feature['properties']['name'] = re.sub(r'\[.*?\]|\?', '', feature['properties']['name']).strip()
#         # do the same for county
#         if 'properties' in feature and 'county' in feature['properties']:
#             feature['properties']['county'] = re.sub(r'\[.*?\]|\?', '', feature['properties']['county']).strip()
...
#                             ], md=6),
#                         ]),
#                         # --- new row for combined mode checkbox --- 
#                         dbc.Row([
#                             dbc.Col([
#                                 dbc.Checkbox(
#                                     id='combined-mode-toggle',
#                                     label="Combined Code Mode",
#                                     value=False,
#                                     className="mb-3 mt-2"
#                                 )
#                             ], md=6)
